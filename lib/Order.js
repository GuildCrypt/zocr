...
module.exports = class Order {
  ...
  async fetchMakerAssetFillable() {
    const isCancelled = await this.fetchIsCancelled()
    if (isCancelled) {
      return new Amorph(new Uint8Array([]))
    }
  ...
  async fetchIsCancelled() {
    if (this.isCancelled === true) {
      return this.isCancelled
    }
    const exchange = this.pair.zocrProviderClient.getExchange()
    const hash = await this.fetchHash()
    const isCancelledAmorph = await exchange.fetch('cancelled(bytes32)', [hash])
    const isCancelled = isCancelledAmorph.to(amorphBoolean)
    if (isCancelled) {
  ...
  async cancel() {
    ...
    const transactionHashHexPrefixed = await zeroExContractWrappers.exchange.cancelOrderAsync(zeroExOrder)
    return Amorph.from(amorphHex.prefixed, transactionHashHexPrefixed)
